...
function chengecarno(thisinput) {
    $.ajax({
        ...
        success:function (date) {
            ...
            if (data.length>1){
                for (var i=0;i<data.length;i++){
                    html +="<input type='radio' name='order.ordercarid' value='"+data[i]+"'>"+data[i]+"";
                }
                $("#carnotd").html(html);
            }else {
                html +="<input type='text' name='order.ordercarid' value='"+data[0]+"' readonly/>";
                $("#carnotd").html(html);
            }
    ...
    $.ajax({
        url:"sell/query_json_getWwarehousebyCarModel?name="+thisinput.value,
        success:function (date) {
            var data = JSON.parse(date);
            var html = "";
            for (var i=0;i<data.length;i++){
  ...
  <table class="maintable" width="97%" border="0" cellspacing="0">
    ...
    <tr>
      ...
      *<div id="test_names" style="display:none ">请填写订单号</div>      </td>
      <td width="15%"><div align="center">客户</div></td>
    ...
    <tr>
      ...
      <td id="carnotd"><input type="text" id="linkPhone" name="order.ordercarid" id="carno" value="请选择车名和车类型"/></td>
    ...
    <tr>
      ...
      <td><div align="center">提车仓库</div></td>
      <td>
          <select id="warehouse" type="date" name="order.car.warehouseid">
              <option value="-1">请先选择车类型</option>
          </select>
      </td>
    </tr>
